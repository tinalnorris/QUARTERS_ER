...
//function myFunction (){
   //var ci_test =  y_ses.toString().split(',');
   //var lcl_test = ci_test[0];
   //var ucl_test = ci_test[ci_test.length - 1];
    //var lcl_num =  [lcl_test*1];
    //var ucl_num =  [ucl_test*1];
//}


function barPlotData(x_values, y_values, y_ses, y_labels, lcl_test,ucl_test, lcl_num, ucl_num, showSEs, hideYaxis) {
    var plotdata = [];
    ...
    //   var ses_i = selectIndexes(y_ses, keep_i);
    // var ci_test_i = selectIndexes(ci_test, keep_i);
    ...
    // var ci_test_j = selectIndexes(transpose(ci_test_i), keep_j);
   ...
   var lcl_num = ses.slice(0,1).toString().split(',').map(Number);//works but breaks when Q4 and subgroup tooltips
   var ucl_num = ses.slice(-1).toString().split(',').map(Number);//works but breaks when Q4 and subgroup tooltips
    ...
    for(var j = 0; j < y.length; j++) {
      var newtrace = {
        ...
        error_x: {
          ...
          array: lcl_num.map(function(x) {return x;}),
          arrayminus: ucl_num.map(function(x) {return x;}),
...
//        return {data: plotdata, ylabels: newarray};
}
